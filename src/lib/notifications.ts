...
export async function sendContactNotifications(
  ...
  if (emailResult.status === 'rejected') {
    console.error(
      '[Notifications] Email notification threw error:',
      emailResult.reason
    )
  } else if (emailResult.value === false) {
    console.warn('[Notifications] Email notification returned failure (see [Email] logs for details)')
  }

  if (discordResult.status === 'rejected') {
    console.error(
      '[Notifications] Discord notification threw error:',
      discordResult.reason
    )
  } else if (discordResult.value === false) {
    console.warn('[Notifications] Discord notification returned failure (see [Discord] logs for details)')
  }
}
